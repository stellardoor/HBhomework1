...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    items[:] = list(items[:: -1])
    # return items <---- oops read directions
...
def duplicates(items):
    ...
    #         # not_dupe_list.append(item)
    #         continue
    #     else:
    #         dupe_list.append(item)
    # THIS TOOK FOREVERRRRRRRRRRR because I can't read instructions lol
    for item in items:
        if items.count(item) > 1:
            dupe_list.append(item)

    # items[:] = list(set(dupe_list))
