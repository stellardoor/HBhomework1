...
def sign_and_parity(number):
    number_details = []
    if float(number) >= 0:
        number_details.append("Positive")
    elif float(number) < 0:
        number_details.append("Negative")
    if float(number) % 2 == 0:
        number_details.append("Even")
    elif float(number) % 2 != 0:
        number_details.append("Odd")
    return number_details

sign_parody = sign_and_parity(-5)
sign = sign_parody[0]
parity = sign_parody[1]
print(sign)
print(parity)
